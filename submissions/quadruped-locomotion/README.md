...
> **A Go2 that feels the ground.** Five proprioceptive feedback loops keep the trunk
...
See [`demo.mp4`](demo.mp4) (locomotion patrol), [`demo_mission.mp4`](demo_mission.mp4)
(autonomous go-to-goal mission), and [`demo_cargo.mp4`](demo_cargo.mp4) (loco-manipulation
delivery) — generated by `python run.py record` / `record-mission` / `record-cargo`.
...
- **Five closed-loop feedback loops, integrated** — one controller fuses (1) a CPG trot,
  (2) **IMU + gyro heading-hold**, (3) **contact-gated IMU terrain-leveling**, (4) a
  **velocimeter body-velocity loop**, and (5) **go-to-goal position control**, all on
  proprioception (orientation, yaw-rate, foot contacts, body velocity, pose) over the full
  rigid-body dynamics. *True closed-loop integration.*
- **Autonomous, self-verifying mission** — go-to-goal navigation runs a **3-waypoint patrol
  (out → across → home)**, steering on pose and self-verifying arrival within **25 cm**:
  **mission complete 10/10**, all 30 waypoints reached. Agentic autonomy, not a scripted path.
- **Closed-loop speed tracking (uses the velocimeter)** — a PI loop on measured body velocity
  tracks a commanded speed setpoint to **within ~1%** (0.20/0.35/0.50 → 0.198/0.348/0.499 m/s),
...
**Autonomous go-to-goal mission (5th loop, self-verifying):** a 3-waypoint patrol (out → across →
home) steered on pose — **mission complete 10/10**, all 30 waypoints reached within the **25 cm
goal gate** (the arrival threshold), upright 0.97. Autonomy with self-verified arrival, not a
scripted path. *(Runs on flat ground; the autonomy loop is separate from the terrain showcase.)*
...
control, and **four integrated sensor-feedback loops** (heading-hold + terrain leveling +
body-velocity + go-to-goal position) — running on the **full rigid-body dynamics** of a 19-DOF
floating-base quadruped (`nq=19`, `nu=12`) at an explicit **2 ms (500 Hz)** timestep. All loops are validated under
...
track a commanded speed setpoint `v*`. It tracks **0.20 / 0.35 / 0.50 m/s to within ~1%**
(achieved 0.198 / 0.348 / 0.499) across mass/friction randomization — turning the open-loop speed *map* into a true closed-loop speed
...
**4d · Go-to-goal navigation & autonomous mission (5th loop, on pose).** A fifth loop closes on
**position**: given a waypoint, it steers the heading toward it (`turn = clip(−k·ψ_err, ±0.22)`),
turning nearly in place when badly mis-aligned and walking once aligned, slowing as it nears, and
**self-verifies arrival** within 0.25 m. Chained over a **3-waypoint patrol (out → across → home)**
it completes the **mission 10/10** (all 30 waypoints, each reached within the 25 cm goal gate) —
autonomous, self-verifying behavior, not a time-scripted path (`goto_mission`). Runs on flat ground.
...
| Record-mission | `python run.py record-mission` | Renders the **autonomous go-to-goal mission** clip (`demo_mission.mp4`) |
...
python run.py record-mission   # regenerate the autonomous-mission clip
python run.py record-cargo     # regenerate the cargo-delivery clip
pytest -q tests/               # 13 tests: real-physics, heading-hold, terrain-fb, push, speed, cargo, …
```
...
assets/
  ...
  mission.xml    # flat scene + 3 visible goal markers for the go-to-goal mission demo
  go2/           # vendored Unitree Go2 model + meshes (BSD-3, LICENSE incl.)
figures/         # hero.png, ablation.png, cargo.png (generated)
tests/           # 13 tests incl. real-physics, heading-hold, terrain-fb, push, speed-tracking, cargo, sensor-noise
```
...
|---|---|
| Reproducibility | one-line install, deterministic, headless, pytest (13 tests), pinned deps, no GPU |
...
| Task design | terrain traversal **+ spin + speed sweep + steering + push-recovery + payload delivery + an autonomous self-verifying waypoint mission**, quantified over randomized trials with ablation & disturbance/delivery/speed/noise/mission probes |
| Control | **true closed-loop integration of 5 loops** — CPG trot + PD torque + heading-hold (IMU+gyro PD) + contact-gated terrain leveling + velocimeter body-velocity tracking + **go-to-goal position control** — surviving shoves, tracking a speed setpoint, transporting a payload, and running an autonomous mission, all under sensor noise |
| Dexterity / motion-richness | spin-in-place, closed-loop speed setpoint tracking, go-to-goal navigation, turns, **and loco-manipulation: delivering a free rigid-body payload over terrain** |
| Engineering quality | typed modules, 13 tests (physics-authenticity, ablation, push, speed-tracking, cargo, sensor-noise, go-to-goal), persisted metrics JSON, vendored license |
| Presentation | two annotated demos (locomotion + cargo delivery) with rich HUD + generated-from-data hero/ablation/cargo figures |
| Innovation | five integrated proprioceptive loops + an autonomous self-verifying mission + measured disturbance rejection + loco-manipulation + real-world sensor-noise robustness on a *dynamic* gait, where the baseline example is kinematic animation |
