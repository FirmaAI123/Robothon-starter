...
See [`demo.mp4`](demo.mp4) — a concise highlights reel (**harder terrain → shove recovery →
payload delivery**) — plus [`demo_cargo.mp4`](demo_cargo.mp4) (detailed cargo run). Generated by
`python run.py record` / `record-cargo`.
...
- **Generalizes to harder terrain** — the *same* controller (no retuning) on a tougher course
  (steeper ramp + bigger rough ±4.5 cm + taller step + a downhill + a cross-slope) stays
  **upright 92 %** (11/12) and pushes ~5 m in, clearing the core hard section 58 % of the time —
...
### Harder terrain (generalization, same controller)

The *same* controller — no retuning — on a deliberately tougher course: a **steeper ramp**, a
**bigger rough patch (±4.5 cm)**, a **taller step**, a **down-ramp**, and a **cross-slope** (side tilt).

![Go2 on the harder course](figures/hard.png)

| metric (`python run.py eval` → hard, 12 randomized trials) | result |
|---|---|
| stayed upright (no fall) | **11/12 = 92 %**  (95 % Wilson CI [65 %, 99 %]) |
| cleared the core hard section (x > 4.7 m) | **7/12 = 58 %**  (95 % CI [32 %, 81 %]) |
| distance into the course | **4.96 ± 0.81 m** |

This is genuinely hard — it does **not** clear every run, and that is reported as-is. The point is
that a controller tuned on the gentle course **generalizes**: it stays upright on terrain well beyond
its tuning envelope and pushes most of the way through, rather than failing at the first new obstacle.
...
| Record | `python run.py record demo.mp4` | Renders the **highlights reel** (harder terrain → shove recovery → payload delivery) |
| Record-cargo | `python run.py record-cargo` | Renders the detailed **loco-manipulation** clip (`demo_cargo.mp4`) |
| Record-patrol | `python run.py record-patrol` | Renders the full locomotion patrol (terrain + spin + speed + turns + push) |
| Eval | `python run.py eval 20` | **Quantitative** robustness + ablation + speed + push + delivery + hard terrain, 95 % CIs → `eval_results.json` |
...
python run.py record demo.mp4  # regenerate the highlights reel
python run.py record-cargo     # regenerate the cargo-delivery clip
pytest -q tests/               # 11 tests: real-physics, heading-hold, terrain-fb, push, speed, cargo, hard
```
...
quadloco/
  ...
  record.py      # demo renderers (highlights reel + cargo + full patrol)
assets/
  ...
  hard.xml       # harder course: steep ramp + big rough + step + down-ramp + cross-slope
  go2/           # vendored Unitree Go2 model + meshes (BSD-3, LICENSE incl.)
figures/         # hero.png, ablation.png, cargo.png, hard.png (generated)
tests/           # 11 tests incl. real-physics, heading-hold, terrain-fb, push, speed, cargo, hard
```
...
|---|---|
| Reproducibility | one-line install, deterministic, headless, pytest (11 tests), pinned deps, no GPU |
| MuJoCo depth | 12 torque actuators, real contacts/friction over uneven + harder terrain (ramp/rough/step/down-ramp/cross-slope), IMU + gyro + contact sensing, `data.xfrc_applied` disturbances, a welded-tray + free-payload body, `mj_step` |
| Task design | terrain traversal **+ a harder course + spin + speed sweep + steering + push-recovery + payload delivery**, quantified over randomized trials with ablation & disturbance/delivery/hard-terrain probes |
...
| Engineering quality | typed modules, 11 tests (physics-authenticity, ablation, push, speed, cargo, hard terrain), persisted metrics JSON, vendored license |
| Presentation | a concise highlights reel + detailed cargo demo, rich HUD + generated-from-data hero/ablation/cargo/hard figures |
