...
def walk_trial(seed, secs=10.0, randomize=True):
    ...
    env.close()
    return {"dist": dist, "speed": dist / secs, "min_up": min_up, "fell": fell, "drift": drift}
...
def evaluate(trials=20, secs=10.0, out_json=None) -> Dict:
    ...
    print(f"\n=== QuadLoco evaluation: {trials} randomized trials over the TERRAIN course "
          f"(ramp+rough-heightfield+step; trunk mass ±12%, heading ±0.05 rad, {secs:.0f}s) ===")
    ...
    print(f"lateral drift (IMU heading-hold): {drifts.mean():.2f} ± {drifts.std():.2f} m")
    summary = {
        ...
        "drift_m": {"mean": float(drifts.mean()), "std": float(drifts.std())},
    }
    if out_json:
